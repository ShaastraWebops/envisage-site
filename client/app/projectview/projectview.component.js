  ...
  constructor($scope,$stateParams,$state,$http) {
     ...
     else if($stateParams.id == "Description 3")
     {
       $scope.projects =
          ...
          desc:"A completely new concept for Shaastra in its entirety (we hope :P). As the name suggests this project will involve coordinators building a bot which can paint a large picture in a very short time in front of a live audience. The coordinators working on this project will be working on Precise motor and actuator control (Painting the picture) ,Power distribution and management (Controlling and powering many actuators at once),Mechanical design (Designing and building the platform on which all these can be placed),Image processing (Making certain changes to a picture so our bot can paint it).",
     ...
     else if($stateParams.id == "Description 4")
     {
       $scope.projects =
          ...
          desc:"Did the thought of making music out of thin air ever crossed your mind? What if you come across some nice music at some random place and you want to play it then and there. Virtual instruments, as we call them, are musical instruments without any physical structure, but you can play the instrument as if it's before you(virtually present). We present it as a band with around 5 or 6 instruments. The challenge here is to make them close to the real ones both in terms of the music they produce and the way they're played.",
     ...
     else if($stateParams.id == "Description 6")
     {
       $scope.projects =
          ...
          members: [],
          desc: "Projection mapping, also known as spatial augmented reality, is a technology which is used to turn irregularly shaped objects into surfaces for projection.By using softwares (Audacity, After Effects to name a few). A two- or three-dimensional object is spatially mapped on the virtual program which mimics the real environment it is to be projected on. The software can interact with a projector to fit any desired image onto the surface of that object.It is an enchanting visual display that mesmerises the audience by a combination of artistic choreography and holographic projections interacting on stage to create a memorable multimedia performance",
     ...
     else if($stateParams.id == "Description 9")
     {
       $scope.projects =
          ...
          desc:"The basic idea for this project is to make a pencil sketcher using two stepper motors which control the two degrees of freedom (X,Y positions). The feed to sketch is to be given from the processed images from the camera. The skills which that are required for this project are 1) Arduino stepper controlling, 2) Basic mech funda, 3) Basic image processing, 4) Gen elec funda",
